function [Chain,Chain0,Chain500,Chain1000,Chain2000,Cells_index_vect,WaveSpeed,Density_history,c2,c,WaveSpeed_vect] = SS_Stochastic_Function_Non_Proliferative(T,alpha,v,Chain,tau)
%     profile on
...
%TO REPRODUCE MAT'S PAPER RESULTS

        if t_previous<=100 && t>100
            Chain500=Chain;
        end
        if t_previous<=500 && t>=500
            Chain1000=Chain;
        end
        if t_previous<=1000 && t>=1000
            Chain2000=Chain;
        end
        
        
%         if floor(t_previous)<500 && floor(t)>500
%             Chain500=Chain;
%         elseif floor(t_previous)<1000 && floor(t)>1000
%             Chain1000=Chain;
%         elseif floor(t_previous)<2000 && floor(t)>2000
%             Chain2000=Chain;
%         end
