...
%     j
if model==1
    [Chain_out1,Chain0,Chain500,Chain1000,Chain2000,Cells_index_vect,WaveSpeed,Density_history,c2,c,WaveSpeed_vect] = SS_Stochastic_Model_Function(T,alpha,v,mu,Chain1,tau);
...
if model==1
    ...
    Y_obs3=max(abs(gradient(Chain_avg1(:,50))));
elseif model==2
    Y_obs2=AveragePropagationSpeed(Chain_avg2,T);
    Y_obs3=max(abs(gradient(Chain_avg2(:,50))));
end
...
for trial=1:N
    ...
    ['Tumor Size: ' num2str(SS_win1) ', ' num2str(GOG_win1) ', ' num2str(draws1) newline 'Wave Speed: ' num2str(SS_win2) ', ' num2str(GOG_win2) ', ' num2str(draws2) newline 'Wave Slope: ' num2str(SS_win3) ', ' num2str(GOG_win3) ', ' num2str(draws3)]
        ...
        %Short-term
        alpha_hat1=0.02*rand(); %sample from prior for SS model
        v_hat1=2*rand();
        alpha_hat2=0.02*rand(); %sample from prior for GoG model
        v_hat2=2*rand();
        ...
        parfor i = 1:2
            ...
            if i == 1
                for k=1:sim
                      [Chain_out1,Chain0,Chain500,Chain1000,Chain2000,Cells_index_vect,WaveSpeed,Density_history,c2,c,WaveSpeed_vect] = SS_Stochastic_Model_Function(T,alpha_hat1,v_hat1,mu,Chain1,tau)
