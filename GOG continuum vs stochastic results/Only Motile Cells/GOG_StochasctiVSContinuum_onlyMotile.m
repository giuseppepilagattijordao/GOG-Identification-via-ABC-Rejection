...
for j=0:dt:CellCycles %time iterations
    j
    if j-dt<=100 && j+dt>=100
        p50=p;
        m50=m;
    end
    if j-dt<=500 && j+dt>=500
        p100=p;
        m100=m;
    end
...
[M2,P2,Chain_out,Chain50,Chain100,Chain200,Q_ij,WaveSpeed,density_cellcycles,WaveSpeed_vect]=Gerlee_Main_loop(Chain,alpha,v,q_p,q_m,mu,CellCycles,0);
% imagesc(Chain)
% title(['Sim=' num2str(i)])
% pause(0.1)
